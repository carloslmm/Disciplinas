...
%if there are only two inputs, it settles AWGN mode
if (nargin<3)
    AWGN_or_Rayleigh='AWGN';  
end

if (lower(AWGN_or_Rayleigh(1))=='a') %Cheks if the first letter of the thir input its 'a'
    ber = a*Q(sqrt(b*10.^(EbN0dB/10))); %AWGN case
else
    rn= b*10.^(EbN0dB/10)/2; 
    ber = 0.5*a*(1-sqrt(rn./(rn+1))); %Rayleigh case
end